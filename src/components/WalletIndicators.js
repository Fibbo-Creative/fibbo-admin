...
export const WalletIndicators = () => {
  ...
  const { getOldManagerBalance } = useApi();
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (walletBalance === 0) {
        ...
        const oldBalance = await getOldManagerBalance();
  ...
  return (
    <div className="flex flex-col gap-3 mx-10 my-5 p-4">
      <div className="text-xl md:text-3xl uppercase"> Indicadores Wallet </div>
      <div className="flex flex-col gap-4 md:flex-row md:gap-10">
        <IndicatorCard
          Title={<div>Wallet</div>}
